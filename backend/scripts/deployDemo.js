...
async function deployContract(contractName, args, deployer) {
	...
	fs.appendFileSync('resultDemo.txt', row+'\n');
...
async function main() {
	...
	console.log(`transferOwnership of ArticleNFT contract from deployer to TruthHubDemo contract...`);
	const result2 = await contractArticleNFT.transferOwnership(contractTruthHubDemo.getAddress());
